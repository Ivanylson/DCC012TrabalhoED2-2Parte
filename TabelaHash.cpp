...
TabelaHash::TabelaHash(int size)
{
    float plusSize;
    if(size == 1431490 )
        plusSize=429451;
    ...
    this->currentSize = 0;
    table = new EntradaHash *[tableSize];
...
int TabelaHash::hashCode(int key, int i)
{
    return (key + (i * i)) % tableSize;
}

int TabelaHash::mixKeysRegister(Apontamento *registro) //preHash
{
...
void TabelaHash::insert(Apontamento *registro)
{
    int i = 0;
    ...
    table[hashIndex] = new EntradaHash(hashIndex, registro);
    currentSize++;
}

//     void HashTable::remove(int key);

int TabelaHash::searchFromCodeAndDate(string code, string date)
{
...
int TabelaHash::searchFromKey(int key)
{
    int i = 0;
...
Apontamento *TabelaHash::getRegistroFromTable(int key)
{
    return table[key]->getRegistro();
}

int TabelaHash::mixKeys(string code, string date) //preHash
{
    int i;
    unsigned long int valor = 3; // valores testados 7 , 3
    for (i = 0; i < date.size(); i++)
